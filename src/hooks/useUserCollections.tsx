...
export const useUserCollections = () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          ...
          const newUserCollection: UserCollectionsTable = {
            username,
            liked: likedData,
            watched: watchedData,
          ...
          await supabase.from('user_collections').insert(newUserCollection);
        ...
        const typedData = data as unknown as UserCollectionsTable;
  ...
  const saveUserData = async () => {
    ...
    try {
      const userCollectionData: UserCollectionsTable = {
        username: userId,
        liked,
        watched,
        watchlist,
        ratings,
        id: '' // This will be ignored on upsert due to onConflict: 'username'
      };
      
      const { error } = await supabase
        .from('user_collections')
        .upsert(userCollectionData, {
          onConflict: 'username'
        });
